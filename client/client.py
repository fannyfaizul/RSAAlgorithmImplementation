    ...
    while True:
        ...
        if choice == '1':
            while True:
                sio.emit('get_user_list')
                time.sleep(1)
                print("Pilih user yang akan menerima pesan:")
                for i, user in enumerate(connected_users, start=1):
                    print(f"{i}. {user['username']}")
                
                try:
                    choice = int(input("Masukkan nomer user yang akan menerima pesan: "))
                    recipient_sid = connected_users[choice - 1]['sid']
                    print("Username penerima: ", connected_users[choice - 1]['username'])
                except (ValueError, IndexError):
                    print("pilihanmu salah Pak Anies, pilih yang valid.")
                    continue

                text_to_send = input("Masukkan pesan yang akan dikirim: ")
            ...
            while True:
                sio.emit('get_user_list')
                time.sleep(1)
                print("pilih user yang akan dibuka pesannya:")
                for i, user in enumerate(connected_users, start=1):
                    print(f"{i}. {user['username']}")
                
                try:
                    choice = int(input("Masukkan nomer user: "))
                ...
                print(f"Total pesan yang diterima: {len(received_messages)}")
                ...
                if len(received_messages) != 0:
                    ...
                    for message in received_messages:
                        if message['sender_sid'] == sender_user['sid']:
                            ...
                            print(f"Menerima pesan dari {sender_user['username']}: {decrypted_text}")
